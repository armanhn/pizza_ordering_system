...
<body>
    ...
    if (isset($_POST["submit"])) {
        ...
        if (!empty($_POST['item_price'])) {
            ...
            $sql = "UPDATE item_tbl
                    SET item_price = '$item_price' WHERE item_id = '$item_id'";
        ...
        if (!empty($_POST['item_details'])) {
            $item_details = $_POST['item_details'];
            $sql = "UPDATE item_tbl
                    SET item_details = '$item_details' WHERE item_id = '$item_id'";
            if (mysqli_query($conn, $sql)) {
                echo "Updated data";
            } else {
                echo "something went wrong";
        ...
        if (!empty($_FILES["image"]["name"])) {
            ...
            if (in_array($fileType, $allowTypes)) {
                ...
                $insert = $conn->query("UPDATE item_tbl SET item_img  = '$imgContent' WHERE item_id = '$item_id'");

                if ($insert) {
                    $status = 'success';
                    $statusMsg = "Image updated successfully.";
                } else {
